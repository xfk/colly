...
func (c *Collector) Visit(URL string) error {
	if c.CheckHead {
		if check := c.scrape(URL, "HEAD", 1, nil, nil, nil, nil, nil, true); check != nil {
			return check
		}
	}
	return c.scrape(URL, "GET", 1, nil, nil, nil, nil, nil, true)
}

// Head starts a collector job by creating a HEAD request.
func (c *Collector) Head(URL string) error {
	return c.scrape(URL, "HEAD", 1, nil, nil, nil, nil, nil, false)
}

// Post starts a collector job by creating a POST request.
// Post also calls the previously provided callbacks
func (c *Collector) Post(URL string, requestData map[string]string) error {
	return c.scrape(URL, "POST", 1, nil, nil, createFormReader(requestData), nil, nil, true)
}

// PostRaw starts a collector job by creating a POST request with raw binary data.
// Post also calls the previously provided callbacks
func (c *Collector) PostRaw(URL string, requestData []byte) error {
	return c.scrape(URL, "POST", 1, nil, nil, bytes.NewReader(requestData), nil, nil, true)
}
...
func (c *Collector) PostMultipart(URL string, requestData map[string][]byte) error {
	...
	return c.scrape(URL, "POST", 1, nil, nil, createMultipartReader(boundary, requestData), nil, hdr, true)
}
...
func (c *Collector) Request(method, URL string, requestData io.Reader, ctx *Context, hdr http.Header) error {
	return c.scrape(URL, method, 1, nil, nil, requestData, ctx, hdr, true)
}
...
func (c *Collector) scrape(u, method string, depth int, prev []url.URL, xpath [][]map[string]string, requestData io.Reader, ctx *Context, hdr http.Header, checkRevisit bool) error {
	...
	c.wg.Add(1)
	if c.Async {
		go c.fetch(u, method, depth, prev, xpath, requestData, ctx, hdr, req)
		return nil
	}
	return c.fetch(u, method, depth, prev, xpath, requestData, ctx, hdr, req)
}
...
func (c *Collector) fetch(u, method string, depth int, prev []url.URL, xpath [][]map[string]string, requestData io.Reader, ctx *Context, hdr http.Header, req *http.Request) error {
	defer c.wg.Done()
	if ctx == nil {
		ctx = NewContext()
	}
